...
void RenderPImageAsText(PImage image, float scale) {
  scale *= 10;
  ...
  for (int x = 0; x < width; x+=scale) {
    for (int y = 0; y < height; y+=scale) {
      ...
      //int[] compareVals = {25, 90, 170, 240};
      int[] compareVals = {50, 90, 170, 240};
      
      if ((r + g + b) / 3 < compareVals[0]) {
        c = characters[0];
      }
      if ((r + g + b) / 3 > compareVals[0]) {
        c = characters[1];
      }
      if ((r + g + b) / 3 > compareVals[1]) {
        c = characters[2];
      }
      if ((r + g + b) / 3 > compareVals[2]) {
        c = characters[3];
      }
      if ((r + g + b) / 3 > compareVals[3]) {
        c = characters[4];
      }
...
void RenderScreenAsText(float scale) {
  color[][] finalPixels = new color[width][height];
  loadPixels();
