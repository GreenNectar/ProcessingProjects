...
int numberOfPoints = 60; // How many points on screen
...
color strokeColour = color(220, 255, 0); // Colour of stroke, only when isHue is disabled
float hue = 0; // Starting hue
float hueIncrement = 1f; // How fast the hue increments, larger = faster
int iterationsPerFrame = 1000; // How many times to iterate per frame
boolean drawPoints = false; // Draw points
color pointsColour = color(255); // Points colour...
boolean drawBackground = true; // Draw background
color backgroundColour = color(69); // Background colour...
boolean completeBlankPoints = true; // Fill in the blank points
...
void draw() {
  ...
  for (int it = 0; it < iterationsPerFrame; it++) {
    if (amountOfNumbers < maximumNumbers || maximumNumbers == 0) {
      ...
      // Flip between top and bottom
      int start = 0;
      int end = 0;
      // Flip between top and bottom
      if (value == 0) {
        start = circleResolution;
        end = 0;
      } else {
        start = 0;
        end = circleResolution;
      }
      ...
      for (int i = start; i <= end; i++) {
        float x = r * cos(i * PI/circleResolution);
        float y = r * sin(i * PI/circleResolution);
