  ...
  if (stack.size() == 0) {
    ...
    for (int i = 0; i < pathValue; i++) {
      ...
      if (i == 0) {
        ...
        //print(255.0 * (float(i)/stackBiggest.size()) + "\n");
        //float value = 1.0 - (float(i)/stackBiggest.size());
        //float r = map(value, 0, 1, 255, 0);
        //value = float(i)/(stackBiggest.size());
        //float g = map(value, 0, 1, 255, 0);
        //stroke(r, g, 0); //255);

        //float value = float(i);//stackBiggest.size();
        
        colorMode(HSB);

        int amount = 16;
        stroke(map(sin((float(i)/stackBiggest.size())*amount), 0, 1, 255/3, 255 - 255/3), 255, 255); //255);
    ...
    if (startPath == true) {
      if (pathValue < stackBiggest.size() - 1) {
        pathValue++;
      } else {
        startPath = false;
      }
