...
int numberOfPoints = 150; // How many points on screen
int circleResolution = 3; // Resolution of each SEMI circle, not circle
int resolutionIncreaseStep = 0; // How many steps until the resolution increases by 1
int resolutionIncreaseStepStep = 0; // How many steps to increase the step when the step, steps
int maximumNumbers = 0; //Zero is unlimited
float strokeSize = 1.8f; // Stroke for circles and points
boolean isHueEnabled = false; // If stroke uses hue or colour
color strokeColour = color(50, 200, 255); // Colour of stroke, only when isHue is disabled
float hue = 0; // Starting hue
float hueIncrement = 0.1f; // How fast the hue increments, larger = faster
...
float xOffset = 0; // X position offset
...
float value = 0f; // To flip the semi circles
...
float middlePointx = 0.5f; // Where the middle of the circle is
float middlePointy = height/2f; // Middle of screen
int multiplier = 1; // To correct hue
...
void setup() {
  ...
  beginRecord(PDF, "Images/FinalImage " + "Num " + numberOfPoints + " - " +
    "Str " + strokeSize + " - " +
    extraStrings +
  ...
  for (int it = 0; it < iterationsPerFrame; it++) {
    if (amountOfNumbers < maximumNumbers || maximumNumbers == 0) {
      ...
      for (int c = 0; c < numberOfPoints; c++) {
        if (positionNumber == c || previousNumber == c) {
      ...
      if (containsPosition) {
        ...
        float r = (abs(positionNumber - previousNumber) / 2f) * (width / numberOfPoints); // Find radius of the semi circle

        middlePointx = (positionNumber - ((positionNumber - previousNumber) / 2f))  * (width / numberOfPoints) + xOffset; // Find the middle of the semi circle
      ...
      int numbers = 0;
      for (int number : numbersUsed) {
        for (int cn = 0; cn < numberOfPoints; cn++) {
          if (number != cn) {
            number++;
          }
        }
...
void DrawPoints() {
  ...
  if (drawPoints == true) {
    for (int q = 0; q < (width) + 1; q++) {
      stroke(pointsColour);
      ellipse(q * (width / numberOfPoints), middlePointy, strokeSize, strokeSize);
    }
  }
}

void DrawSemiCircles() {
  // Draw the semi circle (singular because reasons)
